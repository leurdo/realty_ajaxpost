...
class RealtyAjaxpost
{
	...
	public function add_post() {
        ...
        $title = sanitize_text_field( $_POST['r_title'] );
        $content =  sanitize_textarea_field( $_POST['r_content'] );
		...
		if ( $post_id != 0 )
		{
			...
			//Add meta fields
            $meta_array = array('r_area' => 'area', 'r_living_area' => 'living_area', 'r_address' => 'address', 'r_floor' => 'floor', 'r_price' => 'price');
            foreach( $meta_array as $form_field => $acf_field ) {
                if ( $_POST[$form_field] ) {
                    $text = sanitize_text_field( $_POST[$form_field] );
...
class RealtyAjaxpostWidget extends WP_Widget {
	...
	public function __construct()
	{
		$widget_ops = array('classname' => 'realty_ajaxpostwidget', 'description' => __('Форма для добавления объектов недвижимости', 'rlt') );
		...
		<div id="realty_text">
			...
			<form id="realty_form" action="" method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<input type="file" class="form-control-file" id="r_image" name="r_image" accept="image/x-png,image/gif,image/jpeg" required>
				</div>
